...
def MediatorConcentration(M, ox_grid, red_grid, uptake_grid, time_growth_update, diff_grid, h):
    ...
    red_grid[:-1][bool_mediator] += mol_secreted[:-1][bool_mediator] * time_growth_update
...
def ChargeTransfer(red_grid, ox_grid):
    ...
    r_ox = ((red_grid[-1]*10**-3)/h**3)*(k)*np.exp((1-a)*charge*(E-E_0)) #rate of oxidation at the electrode surface
    r_red = ((ox_grid[-1]*10**-3)/h**3)*(k)*np.exp(-a*charge*(E-E_0))
...
def life_v7(grid, hours_of_simulation, time_growth_update):
    ...
    for timesteps in range(math.ceil(n_updates)):
        ...
        total_red_conc.append(np.sum(red_grid))
        total_ox_conc.append(np.sum(ox_grid))
    ...
    plotting_data = np.concatenate((t_obs_array,cells_array,lactate_array,total_current_array,direct_current_array,mediator_current_array,maximum_current_array,biofilm_thickness,total_ox,total_red)).reshape(10,length)

    np.savetxt(os.path.join("/Users/Oliver_Leblanc/Desktop/", 'test1.txt'), plotting_data)
